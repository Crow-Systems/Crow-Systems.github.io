  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-surface/90 backdrop-blur-md border-b border-outline-variant/30" role="navigation" aria-label="Main navigation">
      <div className="flex justify-between items-center w-full px-6 md:px-8 py-4 max-w-7xl mx-auto">
        ...
        <button onClick={handleMenuToggle} className="md:hidden p-2 text-on-surface" aria-label="Toggle menu">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
        </button>
      </div>
      <div id="mobile-menu" className="md:hidden hidden border-t border-outline-variant/30 bg-surface">
        <div className="px-6 py-4 space-y-3">
          {items.map((item) => (
            <a key={item.label} href={item.href} className="block text-on-surface-variant hover:text-primary text-sm font-medium">{item.label}</a>
          ))}
        </div>
      </div>
    </nav>
...
function Footer() {
  return (
    <footer className="bg-surface-container-low border-t border-outline-variant/20" role="contentinfo">
